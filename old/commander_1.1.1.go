...
// Simple hash for permutations
func (c *commander) hash(s []int) int {
	h := 0
	for i, n := range s {
		h += c.exp[i] * (n - 1)
		h %= MOD
...
// Check whether permutation in table
func (c *commander) contains_restart(s []int) bool {
	h := c.hash(s)
	for _, n := range c.table[h] {
		if equals(s, n) {
			return true
		}
	}
...
func (c *commander) insert_restart(s []int) {
	h := c.hash(s)
	if !c.contains_restart(s) {
		c.table[h] = append(c.table[h], s)
	}
}

// Delete permutation from table
func (c *commander) delete_restart(s []int) {
	h := c.hash(s)
	for i, n := range c.table[h] {
		if equals(s, n) {
			c.table[h][i] = c.table[h][len(c.table[h]) - 1]
			c.table[h] = c.table[h][:len(c.table[h]) - 1]
			return
		}
	}
}
...
func (c *commander) launch() {
	...
	c.exp = append(c.exp, 1)
	for i := 1; i <= c.m; i++ {
		c.fac = append(c.fac, int(math.Min(float64(OPSPS*c.time+1), float64(c.fac[i-1]*i))))
		c.exp = append(c.exp, int(int64(c.exp[i - 1]) * int64(c.m) % int64(MOD)))
	}

        // Hash base cases
        for i := 1; i <= c.m; i++ {
                c.insert_restart([]int{i})
                if c.settings&(1<<LOG_OUT) != 0 {
	...
	if c.settings&(1<<RESTART_IN) != 0 {
		...
		for scanner.Scan() {
			...
			if kill == 0 {
				...
				for i := 1; i <= c.m; i++ {
					if !contains(p, i) && !c.contains_restart(append(p, i)) {
						c.insert_restart(append(p, i))
					}
				}
			}
