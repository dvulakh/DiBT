	...
	for {
		...
		case nxt := <-com.dispatchChan:
			...
			func() {
				...
				comstr += " " + Params + "-n " + strconv.Itoa(max(com.mxSol - com.sols, 0)) + " -c m=" + strconv.Itoa(com.plen) + " -c k=" + strconv.Itoa(com.prng) + " " + assignment + " " + com.prog
...
func (c *commander) process(result returnInfo, run int) bool {
	...
	if result.uid < 0 {
		if c.diff(result.assignment) == 0 || c.depth > len(result.assignment) {
			...
			for i := 1; i <= c.prng; i++ {
				if c.settings&(1<<Repeat) != 0 || !contains(result.assignment, i) {
					c.jobs++
					c.reslock.Unlock()
		...
		if c.settings&(1<<RestartIn) != 0 {
			...
			for scanner.Scan() {
				...
				if kill >= 0.0 {
					...
					for i := 1; i <= c.prng; i++ {
						if c.settings&(1<<Repeat) == 0 && contains(p, i) {
							continue
						}
						if len(p) == int(c.nwrk) {
							p = append(p, 0)
...
func (cmdr *commander) takeInput(line string) {
	...
	funi := map[rune]func(*int, string){'n': foo1, 'm': foo1, 'l':foo1, 't': foo1, 'd': foo1, 's': foo1, 'D': foo1, 'T': foo1, 'L': foo2, 'S': foo2, 'R': foo2, 'B': foo2, 'N': foo1, 'i': foo1, 'p': foo1, 'g':foo1, 'P':foo2, 'b':foo1, 'f':foo1}
	argi := map[rune]*int{'n': &cmdr.nwrk, 'm': &cmdr.prng, 'l':&cmdr.plen, 't': &cmdr.wtime, 'd': &cmdr.depth, 'L': &cmdr.settings, 's': &cmdr.mxSol, 'D': &cmdr.ndfs, 'T': &cmdr.ctime, 'S': &cmdr.settings, 'R': &cmdr.settings, 'B': &cmdr.settings, 'N': &cmdr.netCode, 'i': &cmdr.wid, 'p': &cmdr.opcap, 'g':&cmdr.gauge, 'P':&cmdr.settings, 'b':&cmdr.binr, 'f':&cmdr.binl}
