...
const Params = "--rand-freq=0.001 -q "				// Extra parameters for clingo worker
const PostProcessor = "perl post.clingo.short.pl"		// Solution-counting and timing post-processor invocation
...
const ConsistencyCheck = "check.go"				// Default consistency check location
...
const NoCheck = 0						// Consistency not checked
const ClingoCheck = 1						// Consistency checked by 1s clingo check
const GoCheck = 2						// Consistency checked by imported go package
...
func (c *commander) runWorker(nxt []int, uid int, run int, time int) ([]byte, error) {
	...
	nsol := max(c.mxSol - c.sols, 0)
	if nsol == 0 && c.mxSol != 0 {
		return nil, nil
	}
	comstr += " " + Params + "-n " + "0"/*strconv.Itoa(nsol)*/ + " -c m=" + strconv.Itoa(c.plen) + " -c k=" + strconv.Itoa(c.prng) + " " + assignment + " " + c.prog
		...
		case nxt := <-com.dispatchChan:
			...
			func() {
				...
				if psterr != nil {
					...
					t, _ := strconv.ParseInt(pstr[0], 10, 32)
					f, _ := strconv.ParseFloat(pstr[1], 64)
					tm = float64(f)
					sol = int(t)
				}
				// Run timer
...
func (c *commander) construct(m int, n int, tm int, ctm int, settings int, mxSol int, resin string, d int, head []int, s int, comp string, prog string, ipString string, netCode int, opcap int, gauge int, check int) {
	// Network information
	c.ipString = ipString
	c.netCode = netCode
	c.wid = -1
...
func defaultCom() commander {
	...
	n := N
	g := G
	c.construct(m, n, t, ct, settings, mxSol, resin, d, head, s, wc, wp, ip, netCode, PermutationsPerSecond, g, 0)
	return c
}
		...
		if c.diff(result.assignment) == 0 || c.depth > len(result.assignment) {
			...
			for i := 1; i <= c.prng; i++ {
				...
				if c.isValid(append(append([]int(nil), result.assignment...), i)) && (c.settings&(1<<Repeat) != 0 || !contains(result.assignment, i)) {
					...
					c.reslock.Lock()
					if c.settings&(1<<RestartOut) != 0 && !(c.settings&(1<<Repeat) == 0 && contains(result.assignment, i)) {
				                fmt.Fprintf(c.res, "%d", len(result.assignment) + 1)
						for _, n := range result.assignment {
						        fmt.Fprintf(c.res, " %d", n)
				                }
						fmt.Fprintf(c.res, " %d", i)
				                fmt.Fprintf(c.res, " 0 0.0\n")
					}
				}
			}
			c.jobs--
...
func (c *commander) launch() {
	...
	Mainloop:
		...
		emptychan(c.dispatchChan)
		c.reslock.Lock()
		c.run++
		c.reslock.Unlock()
		c.bar(c.done, c.jobs)
		lck := 0
		if c.settings&(1<<BarOut) != 0 {
			lck = <-c.drawing
		}
		fmt.Println()
...
func (cmdr *commander) takeInput(line string) {
	...
	foo2 := func(n *int, s string) {
		var v int
		if s[0] == 'L' { v = LogOut } else { if s[0] == 'S' { v = SummaryOut } else { if s[0] == 'R' { v = RestartOut } else { if s[0] == 'B' { v = BarOut } else { v = Repeat } } } }
		if s[1:] == "0" {
			*n &= ^(int(1) << uint(v))
		} else {
			*n |= (int(1) << uint(v))
	...
	funi := map[rune]func(*int, string){'n': foo1, 'm': foo1, 'l':foo1, 't': foo1, 'd': foo1, 's': foo1, 'D': foo1, 'T': foo1, 'L': foo2, 'S': foo2, 'R': foo2, 'B': foo2, 'N': foo1, 'i': foo1, 'p': foo1, 'g':foo1, 'P':foo2, 'b':foo1, 'f':foo1, 'J':foo1, 'c':foo1}
	argi := map[rune]*int{'n': &cmdr.nwrk, 'm': &cmdr.prng, 'l':&cmdr.plen, 't': &cmdr.wtime, 'd': &cmdr.depth, 'L': &cmdr.settings, 's': &cmdr.mxSol, 'D': &cmdr.ndfs, 'T': &cmdr.ctime, 'S': &cmdr.settings, 'R': &cmdr.settings, 'B': &cmdr.settings, 'N': &cmdr.netCode, 'i': &cmdr.wid, 'p': &cmdr.opcap, 'g':&cmdr.gauge, 'P':&cmdr.settings, 'b':&cmdr.binr, 'f':&cmdr.binl, 'J':&cmdr.check, 'c':&cmdr.mxSol}
