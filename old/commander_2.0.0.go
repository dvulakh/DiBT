...
// Report work
func (c *commander) report(tm float64) {
	con := c.connect()
...
func (c *commander) handleRequest(con *net.TCPConn) {
	...
	if set[0] == RETURN_CODE {
		w := return_info{nil, 0, SUCCESS, 0, 0}
		n := parseint(set[1]);
		i := 2;
		for ; i < 2 + n; i++ {
...
func (c *commander) construct(m int, n int, tm int, settings int, mx_sol int, resin string, d int, head []int, s int, comp string, prog string, ip_string string, net_code int) {
	// Network information
...
func (cmdr *commander) take_input(line string) {
	...
	foo1 := func(n *int, s string) {
        ...
        funi := map[rune]func(*int, string) { 'n':foo1, 'm':foo1, 't':foo1, 'd':foo1, 's':foo1, 'i':foo1, 'L':foo2, 'S':foo2, 'R':foo2, 'B':foo2, 'N':foo1 }
        argi := map[rune]*int { 'n':&cmdr.n, 'm':&cmdr.m, 't':&cmdr.time, 'd':&cmdr.d, 'L':&cmdr.settings, 's':&cmdr.mx_sol, 'i':&cmdr.s, 'S':&cmdr.settings, 'R':&cmdr.settings, 'B':&cmdr.settings, 'N':&cmdr.net_code }
...
func main() {
	...
	var c commander
        c.construct(m, n, t, settings, mx_sol, resin, d, head, s, wc, wp, ip, net_code)
	c.take_input(line)
