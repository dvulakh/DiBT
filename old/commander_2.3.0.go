...
func (cmdr *commander) takeInput(line string) {
	...
	funi := map[rune]func(*int, string){'n': foo1, 'm': foo1, 'l': foo1, 't': foo1, 'd': foo1, 's': foo1, 'D': foo1, 'T': foo1, 'L': foo2, 'S': foo2, 'R': foo2, 'B': foo2, 'N': foo1, 'i': foo1, 'p': foo1, 'g': foo1, 'P': foo2, 'b': foo1, 'f': foo1, 'J': foo1}
	argi := map[rune]*int{'n': &cmdr.nwrk, 'm': &cmdr.prng, 'l': &cmdr.plen, 't': &cmdr.wtime, 'd': &cmdr.depth, 'L': &cmdr.settings, 's': &cmdr.mxSol, 'D': &cmdr.ndfs, 'T': &cmdr.ctime, 'S': &cmdr.settings, 'R': &cmdr.settings, 'B': &cmdr.settings, 'N': &cmdr.netCode, 'i': &cmdr.wid, 'p': &cmdr.opcap, 'g': &cmdr.gauge, 'P': &cmdr.settings, 'b': &cmdr.binr, 'f': &cmdr.binl, 'J': &cmdr.check}
