...
const Default = 47						// Default settings: produces all files and status bar, uses map and runs initial worker gauge
...
func (c *commander) gaugeWorker() bool {
	...
	if err != nil && err.Error() == "exit status 30" {
		err = nil
	}
	...
	for {
		...
		if c.netCode == Client {
			c.getWork()
                ...
                for i := 1; float64(i) <= math.Max(float64(c.m), float64(c.gauge)); i++ {
