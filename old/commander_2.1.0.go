...
const WorkerProgram = "costas.clingo"				// Default worker program file location
const WorkerCompiler = "clingo"					// Default worker compiler location
...
const MaxTime = 10						// Default time limit for expected 'easy' tasks
...
const Default = ((1 << 4) - 1) | (1 << 5)			// Default settings: produces all files and status bar
...
type commander struct {
	...
	d		int					// Minimum depth
	m		int					// Dimension of arrays
	n		int					// Number of threads
	s		int					// Number of startup DFS threads
	// Output settings
...
func (c *commander) diff(s []int) int {
	...
	if c.fac[c.m-len(s)] > c.opcap*c.wtime {
		return 0
	}
	return c.wtime
}
...
func (c *commander) handleRequest(con *net.TCPConn) {
	...
	if set[0] == ReturnCode {
		...
		s := "-m " + strconv.Itoa(c.m) + " -T " + strconv.Itoa(c.wtime)
		if req.wid == -1 {
			s += " -i " + strconv.Itoa(c.n)
			req.wid = c.n
			c.n++
		}
		if c.settings&(1<<AssignmentMap) != 0 {
			c.maplock.Lock()
			if c.settings&(1<<LogOut) != 0 {
				fmt.Fprintf(c.log, "Rehashed\tID %d\t%v\n", req.wid, a)
			}
			l, ok := c.safetyHash[toString(a)]
...
func (c *commander) construct(m int, n int, tm int, ctm int, settings int, mxSol int, resin string, d int, head []int, s int, comp string, prog string, ipString string, netCode int, opcap int) {
	// Network information
	c.ipString = ipString
	c.netCode = netCode
	c.wid = -1
...
func defaultCom() commander {
	...
	m := -1
	n := N
	c.construct(m, n, t, ct, settings, mxSol, resin, d, head, s, wc, wp, ip, netCode, PermutationsPerSecond)
	return c
}
...
func (c *commander) process(result returnInfo, run int) bool {
	...
	// Begin restart file entry
        if c.settings&(1<<RestartOut) != 0 && (result.uid >= 0 || c.diff(result.assignment) == 0 || c.d > len(result.assignment)) {
	...
	for {
		...
		if c.netCode == Client {
			c.getWork()
                ...
                for i := 1; i <= c.m; i++ {
                        c.fac = append(c.fac, int(math.Min(float64(c.opcap*c.wtime+1), float64(c.fac[i-1]*i))))
                }
		...
		if c.settings&(1<<RestartOut) != 0 {
			if c.settings&(1<<RestartIn) != 0 {
				...
				c.res, _ = os.Create("restart." + strconv.Itoa(c.netCode) + ".txt")
			}
		}
			...
			default:
				...
				if c.settings&(1<<AssignmentMap) != 0 {
					...
					for k, v := range c.safetyHash {
						if v != nil && v.Back().Value.(*returnInfo).err == Unchecked && v.Back().Value.(*returnInfo).time >= MaxTime {
							if c.settings&(1<<LogOut) != 0 {
...
func (cmdr *commander) takeInput(line string) {
	...
	funi := map[rune]func(*int, string){'n': foo1, 'm': foo1, 't': foo1, 'd': foo1, 's': foo1, 'D': foo1, 'T': foo1, 'L': foo2, 'S': foo2, 'R': foo2, 'B': foo2, 'N': foo1, 'i': foo1, 'p': foo1}
	argi := map[rune]*int{'n': &cmdr.n, 'm': &cmdr.m, 't': &cmdr.wtime, 'd': &cmdr.d, 'L': &cmdr.settings, 's': &cmdr.mxSol, 'D': &cmdr.s, 'T': &cmdr.ctime, 'S': &cmdr.settings, 'R': &cmdr.settings, 'B': &cmdr.settings, 'N': &cmdr.netCode, 'i': &cmdr.wid, 'p': &cmdr.opcap}
	funs := map[rune]func(*string, string){'C': foo3, 'W': foo3, 'I': foo3}
	args := map[rune]*string{'C': &cmdr.comp, 'W': &cmdr.prog, 'I': &cmdr.ipString}
	cmdr.head = []int{}
...
func (cmdr *commander) readInfo(line string, funi map[rune]func(*int, string), argi map[rune]*int, funs map[rune]func(*string, string), args map[rune]*string, funf map[rune]func(*float64, string), argf map[rune]*float64, lch rune, arr *[]int) {
	...
	if cmdr.m < 1 && cmdr.netCode != Client {
		reader := bufio.NewReader(os.Stdin)
...
func main() {
	...
	c := defaultCom()
	c.takeInput(line)
	c.log, _ = os.Create("log." + strconv.Itoa(c.netCode) + ".txt")
	c.sum, _ = os.Create("summary." + strconv.Itoa(c.netCode) + ".txt")
